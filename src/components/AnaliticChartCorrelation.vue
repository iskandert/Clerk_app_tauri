  ...
  <el-card>
    ...
    <div class="content">
      <div class="controls">
        ...
        />
        <el-checkbox v-model="isRelativeSums" label="Относительные значения" />
      </div>
      <div v-if="sums" class="result" :style="correlationBackgroundColor">
        <div class="koef">
          <i style="">ρ</i> = {{ correlation.k.toFixed(2) }}
        </div>
        <div class="title">
    ...
    <div v-if="sums" class="charts-container">
...
const categoriesChecked = computed(() => {
  return Object.fromEntries(
    categoryIds.value.map((id) => [
      id,
      categoriesStored.value.find(({ _id }) => _id === id),
    ])
  );
});
const sums = computed(() => {
...
const correlationBackgroundColor = computed(() => {
  ...
  return (
    prefix + `color-mix(in srgb, ${middleColor} ${100 - diffAbs}%, ${endColor})`
  );
});
...
//   const array1 = [
//     0.37, 0.95, 0.73, 0.6, 0.16, 0.16, 0.06, 0.87, 0.6, 0.71, 0.02, 0.97, 0.83,
//     0.21, 0.18, 0.18, 0.3, 0.52, 0.43, 0.29,
//   ];
//   const array2 = [
//     0.63, 0.04, 0.27, 0.41, 0.85, 0.85, 0.95, 0.14, 0.4, 0.28, 0.98, 0.03, 0.17,
//     0.77, 0.81, 0.83, 0.7, 0.48, 0.57, 0.71,
//   ];

//   const coordsList = array1.map((n, i) => ({
//     x: n,
//     y: array2[i]
//   }));
//   const corr = getPearsonCorrelation(array1, array2);
  ...
  if (Math.abs(correlation.value.k) >= 0.7) {
    ...
    datasets.push({
      label: `Линейная регрессия ${
        regression.mathString
      }, R2 = ${regression.r_2.toFixed(2)}`.replaceAll(".", ","),
      type: "line",
      data: [
